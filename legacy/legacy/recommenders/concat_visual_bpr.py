...
class ConcatVisualBPR(BPR):
    def __init__(
        ...
        super(ConcatVisualBPR, self).__init__(
    ...
    def _build_item_inputs(self, train=True):
        ...
        if train:
            ...
            self._add_input(
                name="item_vfeature",
                dtype="float32",
                shape=[None, self._item_f_source.shape[1]],
                train=False,
            )

    def _input_mappings(self, batch_data, train):

        default_input_map = super(ConcatVisualBPR, self)._input_mappings(
            batch_data=batch_data, train=train
        )
        if train:
            default_input_map[self._get_input("p_item_vfeature")] = self._item_f_source[
                batch_data["p_item_id_input"]
            ]
            default_input_map[self._get_input("n_item_vfeature")] = self._item_f_source[
                batch_data["n_item_id_input"]
            ]
        else:
            ...
            default_input_map[
                self._get_input("item_vfeature", train=False)
        ...
        if train:
            ...
            self._add_module(
                ...
                Concat(
                    ...
                    reuse=True,
                    module_list=[
                        self._get_module("item_lf", train=train),

...
class VisualBPR(BPR):
    ...
    def _input_mappings(self, batch_data, train):
        ...
        default_input_map = super(VisualBPR, self)._input_mappings(
    ...
    def _build_item_extractions(self, train=True):
        ...
        if train:
            ...
            self._add_module(
                "item_vf",
                MultiLayerFC(
                    in_tensor=self._get_input("item_vfeature", train=False),
                    train=False,
                    l2_reg=self._l2_reg_mlp,
                    dims=self._dims,
                    scope="item_visual_embed",

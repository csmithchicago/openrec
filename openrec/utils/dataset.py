...
class Dataset(object):
    def __init__(
        ...
        if implicit_negative:
            ...
            for user_id in self._index_store["positive"]:
                self._index_store["positive_sets"][user_id] = set(
                    self._index_store["positive"][user_id]
                )
            if num_negatives is not None:
                ...
                for user_id in self._index_store["negative"]:
                    self._index_store["negative_sets"][user_id] = set(
                        self._index_store["negative"][user_id]
                    )
        else:
            self._index_store["positive"] = dict()
            self._index_store["negative"] = dict()
        ...
        if self._sortby is not None:
            ...
            for user_id in self._index_store["positive_sets"]:
                self._index_store["positive_sorts"][user_id] = sorted(
                    list(self._index_store["positive_sets"][user_id]),
                    key=lambda item: self._raw_data[
                        self._index_store["positive"][user_id][item]
                    ][self._sortby],
                    reverse=not asc,
                )
    ...
    def is_positive(self, user_id, item_id):
        ...
        if (
            user_id in self._index_store["positive"]
            and item_id in self._index_store["positive"][user_id]
        ):
            return True
        return False
    ...
    def sample_positive_items(self, user_id, num_samples=1):
        ...
        if user_id in self._index_store["positive_sets"]:
            return random.sample(
                self._index_store["positive_sets"][user_id], num_samples
            )
        else:
            return []
    ...
    def sample_negative_items(self, user_id, num_samples=1):
        ...
        if "negative_sets" in self._index_store:
            if user_id in self._index_store["negative_sets"]:
                return random.sample(
                    self._index_store["negative_sets"][user_id], num_samples
                )
            else:
                return []
        else:
            ...
            while len(sample_set) < num_samples:
                if (
                    user_id not in self._index_store["positive_sets"]
                    or sample_id not in self._index_store["positive_sets"][user_id]
                ):

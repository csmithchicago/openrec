...
class ModelTrainer(object):
    ...
    def train(
        ...
        print(
            colored(
                "[Training starts, total_iter: %d, eval_iter: %d, save_iter: %d]"
                % (total_iter, eval_iter, save_iter),
                "blue",
            )
        )
        ...
        for _iter in range(total_iter):
            ...
            if (_iter + 1) % eval_iter == 0:
                print(" " * len("..Trained for %d iterations." % _iter), end="\r")
                print(
                    colored("[iter %d]" % self._trained_it, "red"),
                    "loss: %f" % (acc_loss / eval_iter),
                )
                for sampler in eval_samplers:
                    ...
                    for key, result in eval_results.items():
                        ...
                        if type(average_result) is np.ndarray:
                            print(
                                colored("..(dataset: %s)" % sampler.name, "green"),
                                key,

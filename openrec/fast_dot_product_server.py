...
def _FastDotProductRecommender(batch_size, dim_embed, total_users, total_items):
    ...
    def _add_input(subgraph):
        subgraph["user_lf_cache"] = tf.placeholder(
            tf.float32, shape=[total_users, dim_embed], name="user_lf_cache"
        )
        subgraph["item_lf_cache"] = tf.placeholder(
            tf.float32, shape=[total_items, dim_embed], name="item_lf_cache"
        )
        subgraph["item_bias_cache"] = tf.placeholder(
            tf.float32, shape=[total_items, 1], name="item_bias_cache"
        )

        subgraph.register_global_input_mapping(
            {
                "user_lf_cache": subgraph["user_lf_cache"],
                "item_lf_cache": subgraph["item_lf_cache"],
                "item_bias_cache": subgraph["item_bias_cache"],
            },
            "cache",
        )
    ...
    def _add_usergraph(subgraph):
        user_embedding, _ = LatentFactor(
            id_=None, shape=[total_users, dim_embed], subgraph=subgraph, scope="user"
        )
    ...
    def _add_itemgraph(subgraph):
        item_embedding, _ = LatentFactor(
            id_=None, shape=[total_items, dim_embed], subgraph=subgraph, scope="item"
        )
        item_bias_embedding, _ = LatentFactor(
            id_=None, shape=[total_items, 1], subgraph=subgraph, scope="item_bias"
        )
        subgraph.register_global_operation(
            tf.assign(item_embedding, subgraph["item_lf_cache"]), "cache"
        )
        subgraph.register_global_operation(
            tf.assign(item_bias_embedding, subgraph["item_bias_cache"]), "cache"
        )
...
class FastDotProductServer(object):
    def __init__(
        ...
        total_items,
        dim_embed,
        extract_user_lf_func=None,
        extract_item_lf_func=None,
        extract_item_bias_func=None,
    ):

        assert (
            extract_user_lf_func is not None
        ), "Function for user embedding extraction not specified."
        assert (
            extract_item_lf_func is not None
        ), "Function for item embedding extraction not specified."

        self._model = model
        self._fastmodel = _FastDotProductRecommender(
            batch_size=batch_size,
            dim_embed=dim_embed,
            total_users=total_users,
            total_items=total_items,
    ...
    def _cache(self):
        ...
        for batch_id in range(num_batch):
            input_batch = np.arange(
                batch_id * self._batch_size,
                min((batch_id + 1) * self._batch_size, self._total_users),
            )
            self._user_lf_cache[input_batch] = self._extract_user_lf_func(
                self._model, input_batch
            )
        ...
        for batch_id in range(num_batch):
            input_batch = np.arange(
                batch_id * self._batch_size,
                min((batch_id + 1) * self._batch_size, self._total_items),
            )
            self._item_lf_cache[input_batch] = self._extract_item_lf_func(
                self._model, input_batch
            )
        ...
        if self._extract_item_bias_func is not None:
            ...
            for batch_id in range(num_batch):
                ...
                self._item_bias_cache[input_batch] = self._extract_item_bias_func(
                    self._model, input_batch
                ).reshape((-1, 1))
    ...
    def train(
        self,
        batch_data,
        input_mapping_id="default",
        operations_id="default",
        losses_id="default",
        outputs_id="default",
    ):
    ...
    def save(self, save_model_dir=None, global_step=None):
        ...
        self._model.save(save_model_dir=save_model_dir, global_step=global_step)

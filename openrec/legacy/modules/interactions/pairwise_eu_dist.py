...
class PairwiseEuDist(PairwiseLog):
    ...
    *positive item* :math:`p` and *negative item* :math:`n`, respectively.
    ...
    def __init__(
        ...
        super(PairwiseEuDist, self).__init__(
            user=user,
            item=item,
            item_bias=item_bias,
            p_item=p_item,
    ...
    def _build_training_graph(self):
        ...
        with tf.variable_scope(self._scope, reuse=self._reuse):
            ...
            l2_user_pos = tf.reduce_sum(
                tf.square(tf.subtract(self._user, self._p_item)),
                reduction_indices=1,
                keep_dims=True,
                name="l2_user_pos",
            )
            l2_user_neg = tf.reduce_sum(
                tf.square(tf.subtract(self._user, self._n_item)),
                reduction_indices=1,
                keep_dims=True,
                name="l2_user_neg",
            )
            ...
            self._loss = tf.reduce_sum(
                self._weights * tf.maximum(self._margin - diff, 0)
            )
    ...
    def _build_serving_graph(self):
        ...
        with tf.variable_scope(self._scope, reuse=self._reuse):
            ...
            self._outputs.append(
                2 * tf.matmul(self._user, self._item, transpose_b=True)
                + tf.reshape(self._item_bias, [-1])
                - tf.reshape(item_norms, [-1])
            )

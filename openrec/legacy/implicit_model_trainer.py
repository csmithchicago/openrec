    ...
    *implicit feedback*. 
    ...
    def train(
        ...
        if self._num_negatives is None:
            ...
            print(
                colored(
                    "== Start training with sampled evaluation, sample size: %d =="
                    % num_negatives,
                    "blue",
                )
            )
        ...
        for itr in range(num_itr):
            ...
            if itr % display_itr == 0 and itr > 0:
                ...
                print(
                    colored("[Itr %d]" % itr, "red"),
                    "loss: %f" % (acc_loss / display_itr),
                )
                for dataset in eval_datasets:
                    ...
                    for key, result in eval_results.items():
                        ...
                        if type(average_result) is np.ndarray:
                            print(
                                colored("..(dataset: %s)" % dataset.name, "green"),
                                key,
                                " ".join([str(s) for s in average_result]),
                            )
                        else:
                            print(
                                colored("..(dataset: %s)" % dataset.name, "green"),
    ...
    def _score_full_items(self, users):
        ...
        if self._item_serving_size is None:
            return self._model.serve(
                {"user_id_input": users, "item_id_input": np.arange(self._max_item)}
            )
        else:
            scores = []
            item_id_input = np.zeros(self._item_serving_size, np.int32)
            for ibatch in range(
                int(math.ceil(float(self._max_item) / self._item_serving_size))
            ):
                item_id_list = range(
                    ibatch * self._item_serving_size,
                    min((ibatch + 1) * self._item_serving_size, self._max_item),
                )
                item_id_input[: len(item_id_list)] = item_id_list
                scores.append(
                    self._model.serve(
                        {"user_id_input": users, "item_id_input": item_id_input}
                    )[: len(item_id_list)]
                )
    ...
    def _score_partial_items(self, user, items):
        ...
        if self._item_serving_size is None:
            return self._model.serve(
                {"user_id_input": [user], "item_id_input": np.arange(self._max_item)}
            )[0][np.array(items)]
        else:
            return self._model.serve(
                {"user_id_input": [user], "item_id_input": np.array(items)}
            )[0]
    ...
    def _evaluate_full(self, eval_dataset):
        ...
        for itr in tqdm(
            range(
                int(
                    math.ceil(
                        float(eval_dataset.unique_user_count()) / self._test_batch_size
                    )
                )
            )
        ):
            users = eval_dataset.get_unique_user_list()[
                itr * self._test_batch_size : (itr + 1) * self._test_batch_size
            ]
            scores = self._score_full_items(users=users)
            for u_ind, user in enumerate(users):
                result = self._eval_manager.full_eval(
                    pos_samples=list(
                        eval_dataset.get_interactions_by_user_gb_item(user)
                    ),
    ...
    def _evaluate_partial(self, eval_dataset):
        ...
        for user in tqdm(eval_dataset.get_unique_user_list()):
            ...
            items = self._sampled_negatives[user] + list(
                eval_dataset.get_interactions_by_user_gb_item(user)
            )
            to_be_saved["user_items"][int(user)] = items
            scores = self._score_partial_items(user, items)
            result = self._eval_manager.partial_eval(
                pos_scores=scores[self._num_negatives :],
                neg_scores=scores[: self._num_negatives],
            )
        ...
        if self._eval_save_prefix:
            with open(
                self._eval_save_prefix + "_evaluate_partial.pickle", "wb"
            ) as tmpf:
    ...
    def _exclude_positives(self, eval_datasets):
        ...
        for user in user_set:
            if self._train_dataset.contain_user(user):
                self._excluded_positives[user] = self._excluded_positives[user].union(
                    self._train_dataset.get_interactions_by_user_gb_item(user)
                )
            for dataset in eval_datasets:
                if dataset.contain_user(user):
                    self._excluded_positives[user] = self._excluded_positives[
                        user
                    ].union(dataset.get_interactions_by_user_gb_item(user))

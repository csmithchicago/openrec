    ...
    class _SubGraph(object):
        ...
        def __getitem__(self, key):
            ...
            if self._build_mode:
                assert (
                    self._is_built
                ), "[Build Error] Getting a value from an unconstructed graph."
                return self._port_store[key].retrieve()
            else:
                assert isinstance(self._port_store[key], self._OutPort), (
                    "[Connect Error] Getting a value from the %s in-port" % key
                )
                return self, key
        ...
        def __setitem__(self, key, value):
            ...
            if self._build_mode:
                assert isinstance(self._port_store[key], self._OutPort), (
                    "[Build Error] Assigning a value to the %s in-port" % key
                )
                self._port_store[key].assign(value)
            else:
                assert isinstance(self._port_store[key], self._InPort), (
                    "[Connect Error] Assigning a value to the %s out-port" % key
                )
        ...
        def update_global_input_mapping(
            self, update_input_mapping, identifier="default"
        ):
...
class Recommender(object):
    ...
    def train(
        ...
        if input_mapping_id is None:
            ...
            feed_dict = self._generate_feed_dict(
                batch_data, self.T.get_input_mapping(input_mapping_id)
            )
        ...
        results = self._tf_train_sess.run(
            operations + losses + outputs, feed_dict=feed_dict
        )

        return_dict = {
            "losses": results[len(operations) : len(operations) + len(losses)],
    ...
    def train_inspect_ports(self, batch_data, ports=[], input_mapping_id="default"):
        ...
        results = self._tf_train_sess.run(ports, feed_dict=feed_dict)
        return results
    ...
    def serve(
        ...
        if input_mapping_id is None:
            ...
            feed_dict = self._generate_feed_dict(
                batch_data, self.S.get_input_mapping(input_mapping_id)
            )
        ...
        results = self._tf_serve_sess.run(
            operations + losses + outputs, feed_dict=feed_dict
        )
    ...
    def serve_inspect_ports(self, batch_data, ports=[], input_mapping_id="default"):
        ...
        results = self._tf_serve_sess.run(ports, feed_dict=feed_dict)
        return results
    ...
    def restore(self, save_model_dir=None, restore_train=False, restore_serve=False):
        ...
        if restore_train:
            ...
            with self.traingraph.tf_graph.as_default():
                self._optimistic_restore(
                    self._tf_train_sess, os.path.join(save_model_dir, "model.ckpt")
                )
        if restore_serve:
            assert self._serve is not None, "serve is not enabled."
            with self.servegraph.tf_graph.as_default():
                self._optimistic_restore(
                    self._tf_serve_sess, os.path.join(save_model_dir, "model.ckpt")
                )
    ...
    def build(self):
        ...
        if self._train:
            ...
            with self.traingraph.tf_graph.as_default():
                ...
                self._tf_train_saver = tf.train.Saver(
                    tf.global_variables(), max_to_keep=10
                )
        ...
        if self._serve:
            ...
            with self.servegraph.tf_graph.as_default():
                ...
                self._tf_serve_saver = tf.train.Saver(
                    tf.global_variables(), max_to_keep=10
                )

        if self._init_model_dir is not None:
            self.restore(
                save_model_dir=self._init_model_dir,

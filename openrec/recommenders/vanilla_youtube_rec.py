...
def VanillaYouTubeRec(
    ...
    dropout=None,
    init_model_dir=None,
    save_model_dir="Vanilla_YouTubeRec/",
    train=True,
    serve=False,
):
    ...
    def serve_input_graph(subgraph):
        subgraph["seq_item_id"] = tf.placeholder(
            tf.int32, shape=[None, max_seq_len], name="seq_item_id"
        )
        subgraph["seq_len"] = tf.placeholder(tf.int32, shape=[None], name="seq_len")
        subgraph.register_global_input_mapping(
            {"seq_item_id": subgraph["seq_item_id"], "seq_len": subgraph["seq_len"]}
        )

...
def BPR(
    ...
    def train_input_graph(subgraph):
        subgraph["user_id"] = tf.placeholder(
            tf.int32, shape=[batch_size], name="user_id"
        )
        subgraph["p_item_id"] = tf.placeholder(
            tf.int32, shape=[batch_size], name="p_item_id"
        )
        subgraph["n_item_id"] = tf.placeholder(
            tf.int32, shape=[batch_size], name="n_item_id"
        )
    ...
    def serve_input_graph(subgraph):
        ...
        subgraph.register_global_input_mapping(
            {"user_id": subgraph["user_id"], "item_id": subgraph["item_id"]}
        )
    ...
    @t.itemgraph(
        outs=["p_item_vec", "p_item_bias", "n_item_vec", "n_item_bias"],
    ...
    def serve_item_graph(subgraph):
        _, subgraph["item_vec"] = LatentFactor(
            l2_reg=l2_reg,
            init="normal",
    ...
    @t.interactiongraph(
        ins=["user_vec", "p_item_vec", "p_item_bias", "n_item_vec", "n_item_bias"]
    )
